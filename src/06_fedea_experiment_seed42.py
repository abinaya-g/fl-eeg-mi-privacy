"""
BCI-IV 2a — FEDERATED EUCLIDEAN ALIGNMENT (Fed-EA)
=====================================================
Purpose: A candidate methodological contribution, grounded in this
paper's own diagnosed failure modes. Section 5.2-5.3 traced CORAL's and
FedRA's failures in FL to their need for CROSS-CLIENT communication of
alignment statistics (a noisy approximated reference covariance for
CORAL; an aggregated, wrong-dimensionality Riemannian mean for FedRA).
Euclidean Alignment (EA) [He & Wu, IEEE TBME 2020] does not have this
requirement: each subject's whitening matrix is computed ENTIRELY from
that subject's own trials, with no cross-client statistic ever
communicated or aggregated. This script tests whether a
communication-free alignment method avoids the failure mode that
communication-dependent alignment methods (CORAL, FedRA) exhibited.

Method: for EACH client (including, at evaluation time, the held-out
target subject), compute that subject's own mean trial covariance R_s
from its own available trials, then whiten every trial for that subject
using W_s = R_s^(-1/2). This is applied identically to source clients
(computed from their T-session trials, used during training) and to the
target subject (computed from its own E-session trials, used at
evaluation only -- label-free, but transductive in the same sense
flagged in the related BSPC benchmark paper: EA needs batch access to
the target's own UNLABELLED trials, not its labels).

Pipeline position: bandpass (4-40 Hz) -> EA whitening (per-subject,
using that subject's own trials) -> EMS normalisation (per-epoch,
unchanged from all prior scripts) -> EEGNet.

Protocol: identical genuine LOSO structure verified in
fedavg_global_reverify.py -- 8 source clients' T-session data (now
EA-whitened per-client) trains FedAvg global; the held-out target's
E-session (EA-whitened using its OWN covariance) is used only for final
evaluation, zero labels used at any stage.

Output: fed_ea_results.json with per-subject LOSO accuracy. Compare
directly against the verified FedAvg-global-no-EA baseline (~39.84%,
averaged over 2 seeds) and against centralised (40.26%) and local-only
(36.67%).
"""
...
def load_existing_results():
    path = os.path.join(SAVE_DIR, "fed_ea_results.json")
...
def run_loso(sessions):
    ...
    for target in range(1, N_SUBJECTS + 1):
        ...
        with open(os.path.join(SAVE_DIR, "fed_ea_results.json"), 'w') as f:
...
def print_summary(results):
    ...
    print(f"\n  Compare to (all genuine LOSO, verified this revision):")
    print(f"    FedAvg global, no EA (2-seed avg) : 39.84%")
    print(f"    Centralised EEGNet                 : 40.26%")
    print(f"    Local-only ensemble                : 36.67%")
    print(f"\n  If Fed-EA > ~41-42%: plausible genuine improvement, worth")
    print(f"  significance testing against the no-EA FedAvg baseline.")
    print(f"  If Fed-EA is flat or worse: another honest negative result,")
    print(f"  strengthening the 'alignment doesn't help in FL' thesis.")
...
def main():
    ...
    print(f"\nFull results saved: {os.path.join(SAVE_DIR, 'fed_ea_results.json')}")
