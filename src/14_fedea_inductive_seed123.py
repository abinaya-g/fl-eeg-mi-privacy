"""
BCI-IV 2a — FED-EA, FULLY INDUCTIVE VARIANT, SEED 123 (sanity-check rerun)
================================================================
...
Output: fed_ea_inductive_seed123_results.json with per-subject LOSO accuracy.
...
def load_existing_results():
    path = os.path.join(SAVE_DIR, "fed_ea_inductive_seed123_results.json")
...
def run_loso(sessions):
    ...
    for target in range(1, N_SUBJECTS + 1):
        ...
        with open(os.path.join(SAVE_DIR, "fed_ea_inductive_seed123_results.json"), 'w') as f:
...
def print_summary(results):
    print(f"\n{'='*70}\n  FED-EA, FULLY INDUCTIVE, SEED 123 — LOSO SUMMARY\n{'='*70}")
    ...
    print(f"\n  Compare to:")
    print(f"    Fed-EA, fully inductive, seed 42  : 37.82%  (this project's original run)")
    ...
    print(f"\n  If this seed's result is close to 37.82% (within the ~1-3pp")
    print(f"  variance seen between seeds elsewhere in this project): the")
    print(f"  inductive finding replicates and the 2-seed average can be")
    print(f"  used for a properly powered significance test against")
    print(f"  transductive Fed-EA and against the calibration-sweep fractions.")
...
def main():
    ...
    print(f"\nFull results saved: {os.path.join(SAVE_DIR, 'fed_ea_inductive_seed123_results.json')}")
