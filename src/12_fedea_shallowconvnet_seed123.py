"""
BCI-IV 2a — FED-EA, SHALLOWCONVNET BACKBONE, SEED 123 (sanity-check rerun)
==============================================================================
The seed=42 ShallowConvNet Fed-EA run gave 48.10%, significant against
every baseline but currently the only single-seed headline number in
the paper (every other central result is a verified 2-seed average).
This script reruns the identical protocol with seed 123 instead of 42
(affecting weight initialisation, client data shuffling, and the
train/val split -- EA whitening itself is deterministic given the data,
so it is not reseeded by this change) to bring ShallowConvNet Fed-EA up
to the same 2-seed standard as the EEGNet result.

Output: fed_ea_shallowconvnet_seed123_results.json. Average with the
seed=42 result (48.10%) once complete.
"""
...
def load_existing_results():
    path = os.path.join(SAVE_DIR, "fed_ea_shallowconvnet_seed123_results.json")
...
def run_loso(sessions):
    ...
    for target in range(1, N_SUBJECTS + 1):
        ...
        with open(os.path.join(SAVE_DIR, "fed_ea_shallowconvnet_seed123_results.json"), 'w') as f:
...
def main():
    ...
    print(f"\nFull results saved: {os.path.join(SAVE_DIR, 'fed_ea_shallowconvnet_seed123_results.json')}")
