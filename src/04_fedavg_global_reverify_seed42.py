"""
BCI-IV 2a — FEDAVG GLOBAL RE-VERIFICATION (genuine LOSO protocol)
====================================================================
...
Output: fedavg_global_reverify_results.json with per-subject LOSO
accuracy. Compare the resulting mean directly against the paper's
claimed 46.67% (Table 4).
"""
...
def load_existing_results():
    path = os.path.join(SAVE_DIR, "fedavg_global_reverify_results.json")
...
def run_loso(sessions):
    ...
    for target in range(1, N_SUBJECTS + 1):
        ...
        with open(os.path.join(SAVE_DIR, "fedavg_global_reverify_results.json"), 'w') as f:
...
def main():
    ...
    print(f"\nFull results saved: {os.path.join(SAVE_DIR, 'fedavg_global_reverify_results.json')}")
